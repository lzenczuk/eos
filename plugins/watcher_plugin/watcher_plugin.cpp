...
namespace eosio {
   ...
   public:
      ...
      bool filter( const action_trace& act ) {
          /*std::ostringstream o;
          o << "==============> action: " << act.receipt.receiver << ":" << act.act.name;
          ilog(o.str());*/
      ...
      void on_action_trace( const action_trace& act, const transaction_id_type& tx_id ) {
         if( filter( act ) ) {
             //ilog("==============> transaction accepted by filter");
      ...
      void on_accepted_block(const block_state_ptr& block_state) {
         ...
         fc::time_point btime = block_state->block->timestamp;
          ...
          auto dayes = (fc::time_point::now() - btime).to_seconds()/60/60/24;

          std::ostringstream o;
          o << "==============> block dayes from now: " << dayes;
          ilog(o.str());

         if( age_limit == -1 || (fc::time_point::now() - btime < fc::seconds(age_limit)) ) {

             //ilog("==============> age limit OK: "+btime);
            message msg;
